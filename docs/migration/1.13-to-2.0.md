...
```bash
pnpm add @heron-ws/app-runtime@^2.0.0 @heron-ws/component-api@^2.0.0
```

Bump every `@heron-ws/*` package your app depends on to its `2.0.0`-line
release together, not just `app-runtime` — they're versioned independently,
but this release is coordinated across the packages the SSR work touches.
