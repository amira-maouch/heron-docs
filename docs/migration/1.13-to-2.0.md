...
Before running it for the first time, add its output directory to
`.gitignore`, next to the existing `/dist-app/` entry:
...
`dist-server/` didn't exist pre-migration — it's `build:ssr`'s own output
directory (`vite.ts` defaults it to `<root>/dist-server`), so a `1.13.x`
app's `.gitignore` was never written to expect it. Add the entry now, before
anyone runs a build that creates the folder, or it's one accidental
`git add .` away from committing a full server-render bundle.
